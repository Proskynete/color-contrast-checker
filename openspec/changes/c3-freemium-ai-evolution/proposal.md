...
- **Add subscription billing** (Lemon Squeezy) — Free / Pro ($9/mo) / Teams ($29/mo) tiers with Lemon Squeezy checkout and webhooks ✅ *(billing provider changed from Stripe to Lemon Squeezy)*
...
- `subscription-billing`: Lemon Squeezy-powered Free/Pro/Teams subscription tiers with checkout, webhook handling, and plan enforcement ✅ *(was Stripe)*
...
- **New API routes**: `/api/checks`, `/api/checks/[token]`, `/api/ai-suggest`, `/api/lemonsqueezy/checkout`, `/api/lemonsqueezy/webhook`, `/api/palettes`, `/api/palettes/[id]`, `/api/clerk/webhook` ✅
