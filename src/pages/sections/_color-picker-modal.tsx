...
export const ColorPickerSection = ({
	...
	const handleBlur = () => {
		...
		else if (_value.length !== 3 && _value.length !== 6) setError(`The ${_label} field must have 3 or 6 characters`);
		else if (_value.length === 3) setValue(`${_value[0]}${_value[0]}${_value[1]}${_value[1]}${_value[2]}${_value[2]}`);
		else if (!isValid) setError(`The ${_label} field is invalid.`);
	};

	useEffect(() => {
		const handleClick = (e: MouseEvent) => {
			if (previewButtonRef.current?.contains(e.target as Node) || modalRef.current?.contains(e.target as Node)) return;
			setShow(false);
		};
        ...
        ${show ? 'opacity-1' : 'opacity-0 pointer-events-none'}`}
			...
			<div className="relative">
				...
				{show && (
					<div
						ref={modalRef}
						className="w-full h-[19rem] z-50 fixed grid bottom-0 right-0 p-5 bg-white border transition duration-300 md:max-h-fit mx-auto md:w-72 md:p-3 md:absolute md:rounded-lg md:shadow-lg md:top-full md:left-0 md:mt-1 md:bottom-auto md:right-auto"
					>
						<ColorPickerModal fieldName={fieldName} />
						<Input
							id={`${id}-inside`}
							onKeyDownCapture={handleDownPress}
							onBlur={handleBlur}
